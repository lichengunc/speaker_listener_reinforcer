...
local lambda_to_accuracy = {}

for lambda = 0.1, 0.9, 0.1 do
	-- initialize
	...
	while true do
		...
		local data = loader:getImageBatch('val', opt)	
		...
		for i, sent_id in ipairs(sent_ids) do
			...
			local pos_sc
			local max_neg_sc
			local mode_str = 'ensemble'

			-- ensemble
			local scores = emb_scores + lambda * lm_scores
			...
			print(string.format('lambda: %.2f, %s-th: evaluating [%s] performance using [%s] ... image[%d/%d] sent[%d], acc=%.2f%%', 
				lambda, loss_evals, 'val', mode_str, ix0, ix1, i, accuracy*100.0/loss_evals))
		end
		if data.bounds.wrapped then break end  -- we've used up images
	end
	print(string.format('accuracy = %.2f%%', accuracy/loss_evals*100))

	lambda_to_accuracy[lambda] = accuracy/loss_evals*100
end

for lambda = 0.1, 0.9, 0.1 do
    print(string.format('lambda: %.2f, accuracy: %.2f%%', lambda, lambda_to_accuracy[lambda]))
end
